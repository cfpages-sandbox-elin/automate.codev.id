...
export async function requireAdmin(context) {
  ...
  if (!secretKey) return json(503, { ok: false, error: 'login is not configured' });
  ...
  if (!userId) return json(401, { ok: false, error: 'invalid session' });

  const role = await readUserRole(secretKey, userId);
  if (role !== 'admin') return json(403, { ok: false, error: 'admin access required' });
